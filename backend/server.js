...
app.post('/api/tickets', upload.single('ticketImage'), async (req, res) => {
    const ticketNumber = `Ticket${Date.now()}`; // Unique ticket number
    const description = req.body.description;  // Description from the request body
    ...
    try {
      ...
      // Save the ticket to the MongoDB database
      await newTicket.save();

      res.status(201).json({ message: 'Ticket saved successfully', ticketNumber: ticketNumber });
    } catch (error) {
...
// Route to analyze a ticket
app.get('/api/tickets/analyze/:ticketNumber', async (req, res) => {
    try {
        const ticketNumber = req.params.ticketNumber;
        const ticket = await Ticket.findOne({ ticket_number: ticketNumber });  // Find ticket by ticket number
        ...
        // Call OpenAI API to analyze the ticket
        ...
        ticket.decision = decision;  // Update the decision field
